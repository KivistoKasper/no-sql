...
def q2(db):
    ...
    return db.courses.insert_many(courses)
...
def q4(db):
    ...
    query = {'_id': {'$in': ['DPHS.230', 'KONE.411']}}
    return db.courses.find(query)  
...
def q9(db):
    ...
    return db.courses.update_one(
        ...
        {'$set': {
            'categories': ['A', 'B'],
            'prerequisites': [{'course_id': 'MEI.52000'}]
        }}
    )  
...
def q15(db):
    ...
    return db.courses.find(
        {'prerequisites': {'$elemMatch': {
            'course_id': 'MEI.52000'
        }}},
        {'_id': 0, 'categories': 0, 'ects': 0}
    )  
